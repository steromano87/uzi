...
func (s *ServerTestSuite) TestWriteLogsAndRetrieveEntries() {
	...
	serverStream, err := logsClient.GetRawLogs(context.TODO(), &telemetry.RawLogsStreamRequest{})
	if assert.NoError(s.T(), err) {
		// Cancel the current context after 500 ms
		cancelFuncTimer := time.NewTimer(250 * time.Millisecond)
		go func() {
		...
		if assert.Len(s.T(), retrievedLogEntries, 1) {
			assert.NotEmpty(s.T(), retrievedLogEntries[0].GetContent())
			assert.Contains(s.T(), string(retrievedLogEntries[0].GetContent()), "test message")
		}
	}
}
